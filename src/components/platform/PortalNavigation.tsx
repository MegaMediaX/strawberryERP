  ...
  return (
    <>
      <div className="flex items-center justify-between gap-3 md:hidden">
        <span className="text-sm font-medium text-slate-600 dark:text-slate-300">Portal navigation</span>
      ...
      <nav aria-label="Portal" className={cn("mt-4 border-t border-slate-200 pt-4 dark:border-slate-800 md:block", open ? "block" : "hidden")}>
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {visibleGroups.map((group) => (
            <section key={group.label}>
              <h2 className="mb-2 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">{group.label}</h2>
              <div className="flex flex-wrap gap-2">
                {group.items.map((item) => {
                  const active = isActivePath(pathname, item.href);
                  const disabled = impersonating && item.blockedWhenImpersonating;
                  return disabled ? (
                    <span className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-xs font-medium text-slate-400 dark:border-slate-800 dark:text-slate-600" key={item.href} title="Blocked while impersonating">
                    ...
                    <Link
                      ...
                      className={cn(
                        "inline-flex h-9 items-center rounded-lg px-3 text-xs font-medium transition",
                        active
